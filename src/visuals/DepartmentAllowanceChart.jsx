...
const DEPARTMENT_ENUMS = {
  ...
  Oracle: "Oracle",
  "Infra - Client Services": "Client Services",
  BPS: "BPS",
};
...
const DepartmentAllowanceChart = ({ transformedData, clientName = "" }) => {
  ...
  const dataset = useMemo(() => {
    return departments
      .map((item) => {
        const deptFullName = Object.keys(item)[0].trim(); 
        const data = item[deptFullName];
        if (!data) return null;

        return {
          department: DEPARTMENT_ENUMS[deptFullName] || deptFullName, 
          deptFullName, 
  ...
  const enumToFullNameMap = useMemo(() => {
    const map = new Map();
    dataset.forEach((row) => {
      map.set(row.department, row.deptFullName);
    });
    return map;
  }, [dataset]);
      ...
      xAxis={[
        {
          ...
          valueFormatter: (value, context) => {
            if (context.location === "tooltip") {
              return enumToFullNameMap.get(value) || value;
            }
            return value;
          },
        },
