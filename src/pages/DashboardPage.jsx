...
export default function DashboardPage() {
  ...
  const paginatedClientsData = tableData.slice(
    rows * (currentPage - 1),
    rows * currentPage
  );
  const paginatedDepartmentsData = departmentsData?.slice(
  ...
  /* -------------------------
     TABLE FETCHERS
  ------------------------- */
  const fetchClientsTable = useCallback(
    async (payload, clientStartsWith = "") => {
      try {
        ...
        const dashboardData = res.dashboard
          ? Object.entries(res.dashboard).map(([company, info]) => ({
              company,
              ...info,
            }))
          : [];
  ...
  const fetchDepartmentsTable = useCallback(
    async (payload, departmentStartsWith = "") => {
      try {
        const res = await fetchDashboardDepartmentTable(payload, {
          ...(departmentStartsWith
            ? { department_starts_with: departmentStartsWith }
            : {}),
        });
        const dashboardData = res.dashboard
          ? Object.entries(res.dashboard).map(([department, info]) => ({
              department,
              ...info,
            }))
          : [];
  ...
  const debouncedTableFetch = useCallback(debounce(fetchClientsTable, 500), [
    fetchClientsTable,
  ]);
  const debouncedDepartmentTableFetch = useCallback(
    debounce(fetchDepartmentsTable, 500),
